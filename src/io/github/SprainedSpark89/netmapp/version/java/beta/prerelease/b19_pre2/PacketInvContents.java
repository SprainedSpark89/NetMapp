...
public class PacketInvContents extends Packet {
	...
	public PacketInvContents(List<Class<?>> inputs, List<String> descriptions) {
		...
		this.argsDesc.add("Item NBT Lenght|ReadOrder:4");
		this.argsDesc.add("Item NBT|ReadOrder:5");
		/*
	...
	public void read(ByteBuffer buf, ParsedPacket out, StringBuilder desc, Class<?> arg) {
		...
		short[] nbtSize = new short[length];
		...
		for (int i = 0; i < length; i++) {
		    ...
		    if (itemIDs[i] >= 0) {
		        ...
		        // Try reading NBT safely
		        if (buf.remaining() >= 2) {
		            buf.mark();

		            short nbtLength = buf.getShort();

		            if (nbtLength >= 0 && buf.remaining() >= nbtLength) {
		...
		desc.append("ShortArray[").append(length).append("], ");
		out.values.add(nbtSize);
		desc.append("ShortArray[").append(length).append("], ");
		out.values.add(nbt);
		desc.append("ByteArrayArray[").append(length).append("][], ");
	}
